...
(defun gtags-find-symbol (&optional other-win)
  ...
  (let (tagname prompt input)
    ...
    (gtags-push-context)
    (gtags-goto-tag tagname "s" other-win)))

(defun gtags-find-symbol-other-window ()
  "Input symbol and move to the locations in other window."
  (interactive)
  (gtags-find-symbol t))
...
(defun gtags-find-with-grep (&optional other-win)
  "Input pattern, search with grep(1) and move to the locations."
  (interactive)
  (let (tagname prompt input)
    (setq tagname (gtags-current-token))
    ...
    (if (not (equal "" input)) (setq tagname input))
    (gtags-push-context)
    (gtags-goto-tag tagname (if gtags-grep-all-text-files "go" "g") other-win)))

(defun gtags-find-with-grep-other-window ()
  "Input symbol and move to the locations in other window."
  (interactive)
  (gtags-find-with-grep t))
...
(defun gtags-find-file (&optional other-win)
  ...
  (let (tagname prompt input)
    ...
    (if (not (equal "" input)) (setq tagname input))
    (gtags-push-context)
    (gtags-goto-tag tagname "Po" other-win)))

(defun gtags-find-file-other-window ()
  "Input pattern and move to the top of the file in other window."
  (interactive)
  (gtags-find-file t))
